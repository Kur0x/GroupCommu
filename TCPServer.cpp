...
void TCPServer::StartServer() {
	...
	while (1)
	{
		...
		retval1 = select(server_sockfd + 1, &rfds, NULL, NULL, NULL);
		...
		for (i = 0;i<CLIENT_MAX;i++)
			if (client_fds[i].clientfd>0)
			{
				if (FD_ISSET(client_fds[i].clientfd, &rfds))
				{
					...
					{
						if (tcp_recv_server(client_fds[i].clientfd, client_fds[i].recv_playload, client_fds[i].recv_len) <= 0)
						{
							perror("recv");
							exit(1);
						}
					...
					{
						if (tcp_send_server(client_fds[i].clientfd, client_fds[i].send_playload, client_fds[i].send_len) <= 0)
						{
							perror("send");
							exit(1);
						}
