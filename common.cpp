...
void encrypt(char *in, size_t len, ZZ key) {
    uint8_t _key[256] = {};
    string temp = Cryptography::numberToString(key);
    if (temp.size() >= 256)
        memcpy(_key, temp.c_str(), 256);
    else {
        memcpy(_key, temp.c_str(), temp.size());
        bzero(_key + temp.size(), 256 - temp.size());
    }
    uint8_t *_in = new uint8_t[len];
    uint8_t *_out = new uint8_t[len];
